...
const fetchDownloadCount = () => {
  fetch('https://plausible.io/api/v1/stats/aggregate?site_id=yourdomain.com&period=30d&metrics=events&filters=event:name==Download', {
    headers: {
    ...
    .then(data => {
      const downloadCount = data.results.events.value || 0;
      document.getElementById('download-counter').textContent = `Downloaded ${downloadCount} times`;
    })
    .catch(error => {
      console.error('Error fetching download count:', error);
      document.getElementById('download-counter').textContent = 'Download count unavailable';
    });
};
